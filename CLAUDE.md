...
| `whatsapp` | Twilio WhatsApp ingestion. `POST /api/whatsapp/webhook` only **enqueues** to Firestore `whatsapp_queue`; actual processing (AI, expense registration, replies) lives in the separate `gastos-firebase-functions` repo. Also `link`/`unlink`. See section + `WHATSAPP_FLOW.md`. |
