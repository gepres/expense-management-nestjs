...
| `receipts` | Image processing + Claude Vision OCR. **Auth-required** (`FirebaseAuthGuard`); after OCR, `categoria`/`subcategoria` are refined against the **user's taxonomy** via `InferenceService` (shared `@gastos/expense-ai`, homologated with WhatsApp). OCR usage is `scope:user`. |
...
| `voice` | Expense from voice. **`POST /voice/process-audio`** (multipart audio): Whisper server-side (`OpenAiTranscriptionService`, model from `@gastos/expense-ai` `transcribeModel`) → canonical parse → taxonomy classification — same pipeline as WhatsApp. `POST /voice/process-expense` (text body) kept for compat. After extraction, `categoria`/`subcategoria`/`metodoPago` refined against the user's taxonomy via `InferenceService` (`sin_clasificar` keeps the LLM hint). One quota check covers the audio op (transcribe + parse, `scope:user`). Frontend records with `MediaRecorder` (no browser Web Speech API). Response shape unchanged. |
| `inference` | **Shared classifier adapter** (`InferenceService` + `LearningLogService`). Wraps `@gastos/expense-ai` `classifyExpense` (strict order 1→6: taxonomy → history → LLM-bounded → `sin_clasificar`) injecting Firestore deps + `AnthropicService.classifyAgainstTaxonomy`. **Fase 3**: `getHistory` reads the real `users/{uid}/learning_log` (same query as functions — `tokens array-contains-any`, no `orderBy` → automatic index, no Firestore deploy). `recordOutcome()` writes `inference`/`user_correction` (best-effort) via shared `buildLearningLogDoc` — **byte-compatible with WhatsApp** (a correction on either surface feeds the other). Called post-commit from `ExpensesService.create` only when the expense came from voice/receipt (`dto.origenIA`). Single source of truth with `gastos-firebase-functions`. |
| `shopping-lists` | Shopping lists with items + add-from-free-text. |
| `shared` | Shared expense groups (`shared_groups`): budgets, members, invitations, settlement, insights, export. Controller prefix `/api/shared-groups`. **Receipts F1+F2** (v2.10.0): optional photo attached to budget/expense (`receiptUrl`/`receiptPath`, Firebase Storage path `shared-groups/{groupId}/{expenses\|budgets}/{uid}_{ts}.{ext}`); PRO-gated endpoint `POST :id/extract-receipt` (`@RequirePro()`) downloads the image via fetch, validates the path belongs to the calling groupId, and uses `AnthropicService.extractReceiptForSharedGroup()` (Sonnet Vision, Spanish prompt, validates category against the user's taxonomy) to autofill the form. Counts quota as `shared-receipt-scan`. |
| `dashboard` | Quick summary for the Windows widget (and future mobile widgets). `GET /dashboard/summary?range=today\|yesterday\|week\|month` — TZ-aware (reads `users/{uid}.zonaHoraria`, default `America/Lima`), 5-min in-memory cache per `${uid}_${range}`. Returns period expenses (total, count, top 3 categories in main currency) + accounts (with `bankBalance + cashBalance + total`) + `mixedCurrencies` flag if user has accounts in multiple currencies. No FX conversion; sums only the main currency (default account's). |
| `widget` | Pairing endpoint for the Tauri Windows widget. `POST /widget/issue-token` (rate-limited 5/min) generates a Firebase **custom token** with claim `source: 'widget'`, TTL 1h. The web (`/widget-link` page) calls this and redirects to `gastos://auth?customToken=...`, which the Tauri app captures and exchanges via `signInWithCustomToken`. Full flow in [`docs/WIDGET_AUTH.md`](./docs/WIDGET_AUTH.md). |
...
users/{userId}/
  ...
  └── learning_log/               # Bitácora de aprendizaje (compartida con
                                  #   WhatsApp; read+write Admin SDK, schema
                                  #   vía @gastos/expense-ai buildLearningLogDoc)
...
- `extractReceiptData()` - OCR with Claude Vision (shared prompt/parser)
- `extractExpenseFromText()` - voz/texto → canónico ES (shared prompt/parser)
- `categorizeExpense()` - AI category suggestions (import only)
- `classifyAgainstTaxonomy()` - LLM-bounded category pick (helper tier; step 5b of the shared classifier, used by `InferenceService`)
